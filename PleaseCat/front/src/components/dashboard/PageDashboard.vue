...
<!-- v-if="(getLoginInfo != null)" -->
  <div class="profileView">
    <div id="profileView" >
      <div id="leftPart">
        <!-- user_image로 바꾸기 -->
          <img id="userPhoto" :src='require(`@/assets/images/user/${ getLoginInfo.user_image }`)' alt="catProfile">
          <!-- <img id="userPhoto" :src='`/static/images/user/${ getLoginInfo.user_image }`' alt="my Profile"> -->
      </div>
      <section id="rightPart">
          ...
          <div class="btn-wrap">
            <div class="modal selectCat">
              ...
              >
                ...
                <div style="float:center; margin-bottom: 0px;">고양이 선택</div>
                ...
                <div slot="footer">
                  ...
                  >
                    <div class="btn-circle-border">
                      <img
                        class="btn-circle"
                        :src='require(`@/assets/images/cat/${ nc.cat_image }`)'
                      />
                      <!-- <img
                        class="btn-circle"
                        :src='`/static/images/cat/${ nc.cat_image}`'
                      /> -->
                    </div>
          ...
          <!-- 구별 데이터 구분을 위해 입력하는 곳 -->
            ...
            <p class="main-content" style="margin-bottom: 0px;">전체 고양이 수: {{ totalCatNum }}명</p>
            <p class="main-content" style="margin-bottom: 0px;">전체 유저 수: {{ totalUserNum }}명</p>
              </span>
          </div>
      </section>
    </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('storeUser/storeRank',['rankList', 'myRank']),
    ...
    userSexChartData: function() {
      return {
        ...
        datasets: [
          {
            label: "전체 사용자 성별",
            backgroundColor: ["#41B883", "#E46651", "#D3D3D3"],
            data: [4, 6, this.userList.length - 6]
    ...
    userAgeChartData: function() {
      return {
        hoverBackgroundColor: "red",
        hoverBorderWidth: 20,
        labels: ["0~9세", "10대", "20대", "30대", "40대", "50대+", "미상"],
        datasets: [
          {
            label: "전체 사용자 나이",
            backgroundColor: [
    ...
    catRankChartData: function(rankList, myRank) {
        ...
        // if (myRank.rank_point){ // 리스트의 마지막 칸: 내 데이터가 있으면 넣고 아니면 0 #$##$
        //     rankChartData.push(myRank.rank_point)
        // }else{
        //     rankChartData.push(0)
        // }
        let myRankPoint = 0;
        // if (this.myRank != null){
        //   let myRankPoint = myRank.rank_point;
        // } 
        myRankPoint = 0;
        if (this.myRank != null){
          myRankPoint = this.myRank.rank_point;
          // this.myRank.forEach(rank => {
          //   myRankPoint = rank.rank_point// rankChartData.push(rank.rank_point)
          // });
        }
        
        if (rankChartData!=null){
          if (rankChartData[0] < myRankPoint)
            rankChartData[0] = myRankPoint
        }
        ...
        return {
            ...
            datasets: [
            {
                ...
                data: [rankChartData[0], rankChartData[1], rankChartData[2], myRankPoint] //rankChartData[3]]
                // data: [1,2,3,4]
            },
            // {
            //     label: "# of (좋아요 - 싫어요)",
  ...
  mounted() {
    ...
    if (this.myFollowingCatList != []){
      selected_cat_no = this.myFollowingCatList[0].cat_no
    // this.getRankList({cat_no: this.selected_cat_no});
    // this.getMyRank({cat_no:this.selected_cat_no});    
    }else {
      selected_cat_n = 1
    }
...
//#$#
        ...
        // console.log(this.myRank.rankPoint)
  ...
  methods: {
    ...
    ...mapActions('storeUser/storeRank',["getRankList", "getMyRank"]),
    tagCat(no) {
      // 선택한 고양이 값 받아오기
