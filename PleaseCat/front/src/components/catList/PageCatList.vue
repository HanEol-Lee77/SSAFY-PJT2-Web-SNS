    ...
    <div id="catList" class="page">
        ...
        <div id="listView" v-if="isList">
            <transition-group
                ...
                v-on:leave="leave"
            >
                <CatCardComponent v-for="cat in nearCatList" :key=cat.no  :name=cat.cat_name :img=cat.cat_image :desc1="cat.sex==null?'?':(cat.sex==='남'?'♂':'♀')" :desc2=cat.cat_location :src=cat.no  />
            </transition-group>
        </div>
    </div>
</template>
